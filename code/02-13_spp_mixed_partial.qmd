...
(1|rep/krate_kgha) means krate_kgha nested in rep
#Assuming split-split-plot
spp_rcbd_mix_mod <- lmer(yield_kgha ~ krate_kgha*nrate_kgha*prate_kgha + (1|rep/krate_kgha/nrate_kgha))

If massive structure then leaving smaller size which goes into the residual
...
```{r k rate means}
spp_rcbd_k_means <- emmeans(spp_rcbd_mix_mod, ~krate_kgha)

spp_rcbd_k_means
```
The SE of the whole-plot treatment factor (k rate) is **232**.
- krtae_kgha: SE = 232, df = 8.28


What about for the split-plot treatment factor (n rate)?  

```{r n rate means }
spp_rcbd_n_means <- emmeans(spp_rcbd_mix_mod, ~nrate_kgha)
...
SE of n and k rate are different-- tested with mean square of split plot error (Split plot error df is 18(more df))
...
Points to notice:  
  ...
  because of observations going into the error, test the effect
...
we observe a different letter compared to RCBD mixed with blocks random
